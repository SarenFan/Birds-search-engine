...
class TinhTeCrawler:
    """Crawler for TinhTe.vn forum"""

    def __init__(self, output_file: str = 'data_sample/tinhte_data.jsonl',
                 checkpoint_file: str = 'data_sample/tinhte_checkpoint.json',
                 max_docs: int = 100):
        self.base_url = 'https://tinhte.vn'
        # Popular forums on TinhTe
        self.forums = [
            f'{self.base_url}/forums/tin-tuc-cong-nghe.65/',
            f'{self.base_url}/forums/doi-song-so.194/',
            f'{self.base_url}/forums/dien-thoai-smartphone.19/',
        ]
    ...
    async def get_thread_urls(self, forum_url: str, page: int = 1) -> List[str]:
        ...
        # TinhTe structure - find thread titles
    ...
    async def run(self):
        ...
        logger.info(f"Starting TinhTe crawler (target: {self.max_docs} docs)...")
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"TinhTe Crawling Summary:")
...
async def main():
    async with TinhTeCrawler(max_docs=100) as crawler:
